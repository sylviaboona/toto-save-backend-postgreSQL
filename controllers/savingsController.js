...
const createUserSaving = async (req, res) => {
  ...
  const values = [user_id, savings_id, name_of_child, amount, date];

  try {
    const newSavings = await pool.query(addSavings, values);
    const updateAccount = await pool.query("UPDATE accounts SET currentbalance = currentbalance + $1 WHERE name_of_child = $2 AND user_id =$3", [amount, name_of_child, user_id]);
